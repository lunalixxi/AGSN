...
import AGSN.diagram.part.AGSNDiagramEditorUtil;
...
public class SelectModuleContributionItem extends ContributionItem implements
	 ...
	 */
	private SafetyCaseEditPart argumentEditPart;

	/**
	 * the selected edit part.
	 */
	private AGSNNodeEditPart selectedEditPart;
	private AGSNLinkEditPart selectedLinkEditPart;
	...
	public void fill(Menu menu, int index, boolean isCopy) {
		argumentEditPart = AGSNEditorUtil.getCurrentArgumentEditPart();
		selectedEditPart = AGSNEditorUtil.getFirstCurrentSelectedPart();
		//boolean isGoalNode = (selectedEditPart instanceof GoalEditPart);
		// get current module name
		IFile modelFile = AGSNEditorUtil.getModelFile(argumentEditPart);
		//String moduleName = ModuleUtil.getModuleName(modelFile);

		// get list
		Map<String, String> map = null;
		if (selectedEditPart == null) {
			selectedLinkEditPart = AGSNEditorUtil
...
/*		if (!isGoalNode && !isCopy) {
	...
	private class SelectModuleSelectionAdapter extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent event) {
			Diagram currentDiagram = AGSNEditorUtil.getCurrentDiagram();
			...
			EObject eobj = AGSNEditorUtil.getElement(selectedEditPart);
			if (eobj == null) {
				eobj = AGSNEditorUtil.getElement(selectedLinkEditPart);
			}
			if (eobj instanceof BasicNode) {
				...
				AGSNDiagramEditor editor = AGSNEditorUtil.getCurrentAGSNEditor();
			...
			if (!isModuleNode && isCopy) {
				SafetyCaseEditPart currentArgumentEditPart = AGSNEditorUtil
						.getCurrentArgumentEditPart();
				SafetyCase currentArgument = (SafetyCase) AGSNEditorUtil
...
/*					addCmd.add(new ICommandProxy(moveCommand));
			...
			if (selectedNode != null) {
				oldName = ((Evidence)selectedNode).getAttachment();
			} else {
			...
			ICommand setAttachmentCommand = null;
			if (selectedNode != null) {
...
/*			if (!newFlag) {
				...
				setPublicFlag(AGSNEditorUtil.getCurrentArgumentEditPart());
				IWorkbenchPage workbenchPage = PlatformUI.getWorkbench()
						.getActiveWorkbenchWindow().getActivePage();
				IEditorPart editorPart = (IEditorPart) AGSNEditorUtil
...
/*	private String selectUnknownParameters(String orgParameters,
	...
	public static void doPostProcess(BasicNode node, String attachment) {
		...
		SafetyCaseEditPart currentArgumentEditPart = AGSNEditorUtil
				.getCurrentArgumentEditPart();
		IFile myFile = AGSNEditorUtil.getModelFile(currentArgumentEditPart);
		String myName = node.getName();
		//String myNodeName = ModuleUtil.createNodeReference(myFile, myName);
		//AGSNNodeEditPart currentEditPart = (AGSNNodeEditPart) getNodeEditPart(
				//currentArgumentEditPart, myNodeName, true);

		String oldAttachment = ((Evidence) node).getAttachment();
