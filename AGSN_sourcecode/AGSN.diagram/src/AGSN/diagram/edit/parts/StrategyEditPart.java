...
import AGSN.diagram.edit.policies.StrategyItemSemanticEditPolicy;
...
public class StrategyEditPart extends ShapeNodeEditPart {
	...
	*/
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof StrategyNameEditPart) {
			((StrategyNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureStrategyLabelFigure());
			return true;
		}
	...
	*/
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(AGSNVisualIDRegistry.getType(StrategyNameEditPart.VISUAL_ID));
	}
	...
	*/
	protected void handleNotificationEvent(Notification event) {
		if (event.getNotifier() == getModel()
			...
			if (event.getFeature() instanceof EAttributeImpl
					&& event.getFeatureID(Strategy.class) == AGSNPackage.STRATEGY__ASSESSSTATUS) {
	...
	public class StrategyFigure extends GSNStrategy {
		...
		public void updateBGColor() {
			Strategy strategy = (Strategy) ((Node) StrategyEditPart.this.getModel()).getElement();

			switch (strategy.getAssessstatus()) {
			case NOT_REVIEWED:
