...
import AGSN.diagram.edit.policies.GoalItemSemanticEditPolicy;
...
public class GoalEditPart extends ShapeNodeEditPart {
	...
	*/
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof GoalNameEditPart) {
			((GoalNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureGoalLabelFigure());
			return true;
		}
	...
	*/
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(AGSNVisualIDRegistry.getType(GoalNameEditPart.VISUAL_ID));
	}
	...
	*/
	protected void handleNotificationEvent(Notification event) {
		if (event.getNotifier() == getModel()
			...
			if (event.getFeature() instanceof EAttributeImpl
					&& event.getFeatureID(Goal.class) == AGSNPackage.GOAL__ASSESSSTATUS) {
	...
	public class GoalFigure extends GSNGoal {
		...
		public void updateBGColor() {
			Goal goal = (Goal) ((Node) GoalEditPart.this.getModel()).getElement();

			switch (goal.getAssessstatus()) {
			case NOT_REVIEWED:
