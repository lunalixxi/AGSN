 ...
 * @generated
 */
public class AGSNValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	* @generated
	*/
	private static final String MARKER_TYPE = AGSNDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	*/
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof AGSNDiagramEditor) {
	...
	*/
	public boolean provides(IOperation operation) {
		...
		return view != null && SafetyCaseEditPart.MODEL_ID.equals(AGSNVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					AGSNDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
		...
		*/
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				AGSNDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		*/
		public void refresh() {
			...
			try {
				...
				AGSNDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
		...
		*/
		private String getType(IMarker marker) {
			try {
				...
				AGSNDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
